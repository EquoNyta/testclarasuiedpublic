...
const config_filename = _.shuffle([
   "m_vs_2m_oppositeSide_alarm_targSide_l",
  // "m_vs_2m_oppositeSide_alarm_distSide_l",
   "m_vs_2m_oppositeSide_alarm_targSide_r",
  // "m_vs_2m_oppositeSide_alarm_distSide_r",
])[0];

sp.alarm_filename = _.shuffle(sp.alarm_filename)[0];

// let subjectNo = parseInt(jsPsych.data.getURLVariable('PROLIFIC_PID'), 10);
// if (isNaN(subjectNo)) {
//   subjectNo = 100000;
// }

// // from participant and modality numbers determine which json to load and the alarm to choose (the base have to be precise for parseInt to avoid errors on some navigators)
// let config_filename = (function (subjectNo, sp) {
//   let alarm_type = [];
//   let modality = parseInt(jsPsych.data.getURLVariable("id"), 10);
//   if (isNaN(modality)) {
//     modality = 2;
//   }
...
//   switch (modality) {
//     case 1:
//       modality = ["m_vs_2f", "f_vs_2m"];
//       alarm_type = ["synth_M", "synth_F"];
//       break;
//     case 2:
//       modality = [
//         "m_vs_2m_oppositeSide_alarm_targSide_l",
...
//       //   break;
//     default:
//       throw Error("The modality number is incorrect");
//   }
//   const conditions = allperms(modality, alarm_type);
//   let conditionNo = subjectNo - 1;
//   while (conditionNo >= conditions.length) {
//     // because there is X conditions, we have to traduce the subjectNo
//     conditionNo -= conditions.length;
//   }
//   const config_filename = conditions[conditionNo][0];

//   //for the alarm
//   const substring = conditions[conditionNo][1];
//   sp.alarm_filename.map((e) => {
//     if (!e.includes(substring)) {
//       sp.alarm_filename = arrRemove(sp.alarm_filename, e);
//     }
//   });
//   sp.alarm_filename = _.shuffle(sp.alarm_filename)[0];
  ...
  .then((timeline) => {
    ...
    jsPsych.init({
      ...
      on_finish: function (data) {
        ...
        window.location.href = "https://app.prolific.co/submissions/complete?cc=CLBLTYM4";
      },
    });
  })
  .catch((err) => {
